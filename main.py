...
def plot_histogram_only(hist_counts, hist_range, hist_bins, output_path, title):
    ...
    plt.bar(edges[:-1], hist_counts, width=np.diff(edges), alpha=0.7,
    ...
    if total > 0:
        ...
        left = mean - 2 * std
        right = mean + 2 * std
        plt.axvspan(left, right, color="green", alpha=0.2,
                    label="Interval 95% (±2σ)")
...
if __name__ == "__main__":
    ...
    for pkl_file in pkl_files:
        ...
        # Calculam numarul de simulari necesare
        num_sims = int(np.ceil(np.abs(((b - a) ** 2) / (2 * error * 2) * np.log(2 / (1 - trust))))) + 1

        print(f"Ruleaza Monte Carlo ({num_sims:,} simulari, batch={batch_size:,})")

        res = monte_carlo_simulation_batches(data, total_iterations=num_sims,
        ...
        plot_histogram_only(res["hist_counts"], res["hist_range"],
                            res["hist_bins"], out_path, pkl_file)
